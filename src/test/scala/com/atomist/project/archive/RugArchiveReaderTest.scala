...
class RugArchiveReaderTest extends FlatSpec with Matchers {
  ...
  it should "only describe a single project_name parameter if it's declared" in {
    ...
    val ops = apc.find(new SimpleFileBasedArtifactSource("", Seq(GeneratorWithoutProjectName, EditorWithProjectName, AnotherEditorWithProjectName)), Nil)
  ...
  it should "parse single editor" in {
    val apc = new RugDslArchiveReader(atomistConfig)
    val ops = apc.find(new SimpleFileBasedArtifactSource("", FirstEditor), Nil)
  ...
  it should "parse editor and reviewer" in {
    val apc = new RugDslArchiveReader(atomistConfig)
    val ops = apc.find(new SimpleFileBasedArtifactSource("", Seq(FirstEditor, SecondOp)), Nil)
    assert(ops.editors.size === 1)
    assert(ops.editorNames === Seq("First"))
  ...
  it should "parse editor and reviewer and generator" in {
    val apc = new RugDslArchiveReader(atomistConfig)
    val ops = apc.find(new SimpleFileBasedArtifactSource("", Seq(FirstEditor, SecondOp, Generator)), Nil)
    assert(ops.editors.size === 1)
