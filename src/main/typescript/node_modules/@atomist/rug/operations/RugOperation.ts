...
interface Secret extends BaseSecret{
  readonly name: string
}
...
* Decorator for parameters. Adds to object properties
*/
function secret(details: BaseSecret) {
  return function (target: any, propertyKey: string) {
    var params = get_metadata(target, "secrets");
    if(params == null){
      params = []
    }
    details["name"] = propertyKey
    details["decorated"] = true
    params.push(details);
    set_metadata(target, "secrets", params);
  }
}

export {Secret, RugOperation, Parameter, Pattern, Result, Status, ReviewResult, ReviewComment, Severity, BaseParameter, parameter, editor, generator, reviewer, tags, secret}
