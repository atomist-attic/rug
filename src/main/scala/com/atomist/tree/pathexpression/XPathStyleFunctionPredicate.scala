...
case class XPathStyleFunctionPredicate(override val name: String,
  ...
  def evaluate(tn: GraphNode,
    ...
    val convertedArgs: Seq[Any] = args.zipWithIndex.map(tup =>
      convertToRequiredType(tup._1, fun.argTypes(tup._2), tn, ee, executionContext, nodePreparer))
  ...
  private def convertToRequiredType(fa: FunctionArg, requiredType: XPathType,
                                    ...
                                    ec: ExecutionContext, np: Option[NodePreparer]): Any =
    requiredType match {
      case String => convertToString(fa, contextNode, ee, ec, np)
      case Boolean => convertToBoolean(fa, contextNode, ee, ec, np)
      case _ => ???
    }

  // See XPath spec 4.2
  private def convertToString(fa: FunctionArg, contextNode: GraphNode, ee: ExpressionEngine, ec: ExecutionContext, np: Option[NodePreparer]): String = {
    val value = fa match {
      case s: StringLiteralFunctionArg => s.s
      case rpe: RelativePathFunctionArg => ee.evaluate(contextNode, rpe.pe, ec, np) match {
  ...
  private def convertToBoolean(fa: FunctionArg, contextNode: GraphNode, ee: ExpressionEngine, ec: ExecutionContext, np: Option[NodePreparer]): Boolean = fa match {
