...
class RugTranspiler(config: RugTranspilerConfig = RugTranspilerConfig(),
  ...
  private def editorBody(ed: RugEditor): String = {
    ...
    ts ++= helper.indented(s"""let eng: PathExpressionEngine = project.context().pathExpressionEngine();""")
    ts ++= config.separator
    ...
    for (a <- ed.actions) {
      ts ++= helper.indented(actionCode(ed, a, config.projectVarName))
      ts ++= config.separator
    }
    ts.toString
  }
  ...
  private def actionCode(prog: RugProgram, a: Action, outerAlias: String): String = a match {
    case w: With =>
      withBlockCode(prog, w, outerAlias)
    case roo: RunOtherOperation =>
      rooCode(roo,prog.parameters)
  ...
  private def doStepCode(prog: RugProgram, doStep: DoStep, alias: String): String = doStep match {
    ...
    case wds: WithDoStep =>
      withBlockCode(prog, wds.wth, alias)
  }
