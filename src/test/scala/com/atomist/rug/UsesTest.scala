...
class UsesTest extends FlatSpec with Matchers {
  ...
  it should "call other specified namespace from namespace" in {
    ...
    val foo = create(global).head
    val pe = create(prog, Seq(TestUtils.addressableEditor(foo, _group = "atomist", _artifact = "global"))).head
    assert(pe.name === "Redeploy")
  ...
  private  def exportAllExtraParametersFromImportedEditorMinusOnesThatAreSet(namespace: Option[String]) = {
    val prog =
      """
        ...
        | do replace "some" "bar"
      """.stripMargin
    val ed = create(prog).find(_.name == "Redeploy").get
  ...
  it should "preserve parameter ordering per order of editors" in {
    ...
    val prog2 =
      """
        ...
        | do replace "some" "bar"
      """.stripMargin
    val ed2 = create(prog2).find(_.name.equals("Redeploy")).get
  ...
  private  def exportForOtherwiseEmpty(namespace: Option[String]) {
    val prog =
      """
        ...
        |PackageMove
      """.stripMargin
    val ed = create(prog).find(_.name == "ParameterizePackage").get
    assert(ed.parameters.size === 1)
    val red = ed.asInstanceOf[RugDrivenProjectEditor]
  ...
  it should "allow local arguments to be provided explicitly to run" in {
    val prog =
      ...
      """.stripMargin
    val ed = create(prog).find(_.name.equals("ParameterizePackage")).get
  ...
  it should "reject bogus arguments provided explicitly to run" in {
    ...
    val prog =
      """
        ...
        """.stripMargin
    try {
      create(prog).find(_.name.equals("ParameterizePackage")).get
  ...
  private  def create(prog: String, globals: Seq[AddressableRug] = Nil): Seq[ProjectEditor] = {
    ...
    runtime.create(rugAs,globals).asInstanceOf[Seq[ProjectEditor]]
  }
}
